...
func TestParityGroup3(t *testing.T) {
	...
	// vcf-tobedpe variants (cgkit uses the same flag names as ngsutilsj).
	bedpeCases := []struct {
		name string
		args []string
	}{
		...
		// A bare --score key uses the INFO field. (cgkit also accepts an explicit
