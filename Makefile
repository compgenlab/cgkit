...
LDFLAGS := -X 'github.com/compgenlab/cgio/internal/cmd.Version=$(VERSION)' \
           -X 'github.com/compgenlab/cgio/internal/cmd.GitHash=$(GIT_HASH)'
...
build: $(BIN_DIR)/cgio.darwin_arm64 $(BIN_DIR)/cgio.darwin_amd64 $(BIN_DIR)/cgio.linux_arm64 $(BIN_DIR)/cgio.linux_amd64 $(BIN_DIR)/cgio.windows_amd64.exe

$(BIN_DIR):
	mkdir -p $(BIN_DIR)

$(BIN_DIR)/cgio.darwin_arm64: $(GO_SOURCES) | $(BIN_DIR)
	GOOS=darwin GOARCH=arm64 go build -buildvcs=false -ldflags "$(LDFLAGS)" -o $@ .

$(BIN_DIR)/cgio.darwin_amd64: $(GO_SOURCES) | $(BIN_DIR)
	GOOS=darwin GOARCH=amd64 go build -buildvcs=false -ldflags "$(LDFLAGS)" -o $@ .

$(BIN_DIR)/cgio.linux_arm64: $(GO_SOURCES) | $(BIN_DIR)
	GOOS=linux GOARCH=arm64 CGO_ENABLED=0 go build -buildvcs=false -ldflags "$(LDFLAGS)" -o $@ .

$(BIN_DIR)/cgio.linux_amd64: $(GO_SOURCES) | $(BIN_DIR)
	GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build -buildvcs=false -ldflags "$(LDFLAGS)" -o $@ .

$(BIN_DIR)/cgio.windows_amd64.exe: $(GO_SOURCES) | $(BIN_DIR)
